...
def calc_features(_df, cir_start, cir_end, n_measurements):
    '''
    ...
    #_df = _df.loc[::int(50/n_measurements)]
    # print(f"prior {_df.iloc[0].std_noise= }")
    # print(f"after {_df.iloc[0].std_noise= }")
    ...
    # if "
    #_df['f9'] = _df['rng_wo_bias']
    #_df['f10'] = _df['bias']
    _df['f11'] = _df.apply(lambda x: calc_pre_noise(x),axis=1)
    # _df['f12'] = _df['fp_power'].abs()
    # _df['f13'] = _df['rssi'].abs()
    # _df['f14'] = (_df['f12'] - _df['f13']).abs()
    # _df['FPPL'] = calc_FPPL(_df)
    # _df['FPPL'] = _df.fp_power
    # _df['LOS'] = _df.apply(lambda a: calc_LOS(a),axis=1)
    _df['cir'] = _df['cir_bak']
